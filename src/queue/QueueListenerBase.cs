...
public abstract partial class QueueListenerBase<TConfig>
{
    protected readonly
        ConcurrentDictionary<string, ConcurrentDictionary<Type, ConcurrentHashSet<QueueListenerSubscriber>>>
        subscribers = new();
    ...
    protected virtual async Task DoSubscribe<T, TProxy>(QueueListenerSubscriber subscriber)
    {
        ...
        var byType = subscribers.GetOrAdd(typeHelper.EncodeType(type),
                         ...
                         {
                             createdNew = true;
                             return new ConcurrentDictionary<Type, ConcurrentHashSet<QueueListenerSubscriber>>();
                         })
                     ?? throw new QueueListenerSubscibeException(
                         $"Problem with creating or obtaining list of subscribers for given type [{typeHelper.EncodeType(type)}].");
        var byProxyType = byType.GetOrAdd(tproxy,
                              _ => new ConcurrentHashSet<QueueListenerSubscriber>())
                          ?? throw new QueueListenerSubscibeException(
                              $"Problem with creating or obtaining list of subscribers for given proxy type [{typeHelper.EncodeType(tproxy)}].");
            ...
            if (byType.TryGetValue(tproxy, out var byProxyType))
            {
                ...
                if (toRemove != null)
                    if (byProxyType.TryRemove(toRemove))
                        if (byProxyType.IsEmpty && byType.Remove(tproxy, out var _) && byType.IsEmpty &&
                            subscribers.TryRemove(typeHelper.EncodeType(type), out var _))
                ...
                throw new QueueListenerUnsubscibeException(
                    $"Subscription list for given proxy type [{typeHelper.EncodeType(type)}] doesn't exist");
        else
            throw new QueueListenerUnsubscibeException(
                $"Subscription list for given type [{typeHelper.EncodeType(type)}] doesn't exist");
    }
